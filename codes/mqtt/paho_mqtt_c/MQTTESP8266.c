...
int  mqtt_esp_read(mqtt_network_t* n, unsigned char* buffer, int len, int timeout_ms)
{
    ...
    if ((rc > 0) && (FD_ISSET(n->my_socket, &fdset)))
    {
        ...
        /* Select returns immediately if the broker terminates the connection.        *
         * Since mqqt task has a higher priority, others task can hang indefinitely   *
         * When the connection is terminated by the broker, recv returns 0 bytes      *
         * even when len bytes are requested                                          */
        if (rcvd == 0 && len != 0){
            return -4; // MQTT_READ_ERROR = -4, see MQTTClient.h
        }
